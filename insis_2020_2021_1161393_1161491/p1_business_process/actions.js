...
function setVariablesForMultiSelectKey(variables, key, newValue) {
    const currentVariableForKey = variables.find(variableEntry => variableEntry.name === key);

    if (currentVariableForKey) {
        const currentValue = currentVariableForKey.value;

        const newValueForKey = currentValue + ',' + newValue;
        currentVariableForKey.value = newValueForKey;
    } else {
        variables.push({
            name: key,
            value: newValue
                ...
                for (var j = 0; j < vData.length; j++) {
                    if (vData[j].id == data[i].name) {
                        if (vData[j].required && vData[j].writable && data[i].value == "") {
                            ...
                            } else if (vData[j].type === "multiSelect") {
                                setVariablesForMultiSelectKey(variables, data[i].name, data[i].value);
                            } else if (vData[j].type === "table") {
                                variables.push({
                                    "name": data[i].name,
                                    "value": data[i].value
                                })
                            } else if (vData[j].type === "list") {
                                variables.push({
                                    "name": data[i].name,
                                    "value": data[i].value
                                })
