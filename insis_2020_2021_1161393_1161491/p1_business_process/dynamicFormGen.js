...
function genMultiSelect(data, disabled) {
    var content = '<label>' + data.name + '</label>';
    content += "<select class='form-control' name='" + Encode.forHtml(data.id) + "'multiple size='3'";
    ...
    if (data.value) {
        ...
        for (var i = 0; i < items.length; i++) {
            content += "<option value='" + Encode.forHtml(items[i].id) + "'>"
                + Encode.forHtml(items[i].name) + "</option>";
        }
    }
    content += "</select>"
...
function genTable(data, disabled) {
    ...
    for (var i = 0; i < list.length; i++) {
        content += "<tr>";
        content += "<td style='padding:5px'>" + list[i] + "</td>";
        content += "</tr>";
    }
...
function genList(data, disabled) {
    var list = JSON.parse(data.value);
...
function generateForm(data, disabled) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        } else if (data[i].type == "multiSelect") {
            formContent += genMultiSelect(data[i], disabled);
        } else if (data[i].type == "table") {
            formContent += genTable(data[i], disabled);
        } else if (data[i].type == "list") {
            formContent += genList(data[i], disabled);
